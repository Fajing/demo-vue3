              ...
              <el-collapse-item title="查询条件" name="1">
                ...
                >
                  ...
                  <el-form-item label="接受中外合作">
                    <el-select v-model="queryParam.zhongWai" placeholder="请选择">
                  ...
                  <el-form-item label="意向省份">
                    <el-checkbox
                      v-model="intentionProvCheckAll"
                      :indeterminate="intentionIsIndeterminate"
                      @change="handleIntentionProvCheckAllChange"
                      >全选</el-checkbox
                    >
                    <el-checkbox-group v-model="queryParam.intentionProv" @change="handleCheckedIntentionProvChange">
                      <el-checkbox
...
const intentionProvCheckAll = ref(false)
const intentionIsIndeterminate = ref(false)
...
const handleIntentionProvCheckAllChange = (val: boolean) => {
  queryParam.value.intentionProv = val ? ProvinceList : []
  intentionIsIndeterminate.value = false
}
const handleCheckedIntentionProvChange = (value: string[]) => {
  const checkedCount = value.length
  intentionProvCheckAll.value = checkedCount === ProvinceList.length
  intentionIsIndeterminate.value = checkedCount > 0 && checkedCount < ProvinceList.length
}
...
const formatCheckboxValue = () => {
  ...
  for (const key in checkboxValue.value) {
    if (Object.prototype.hasOwnProperty.call(checkboxValue.value, key)) {
      ...
      queryParam.value.intentionSubAndMaj = (
        queryParam.value.intentionSubAndMaj as string[]
      ).concat(values)
    }
  }
}
...
const formatQueryParam = (): QueryParam => {
  ...
  //  985高校 转成 985
  let changeSchoolAttr: string[] = []
  ;(queryBody.schoolAttr as String[]).forEach((element) => {
    changeSchoolAttr.push(UniversityNatureList.find((item) => item.name === element)?.key ?? '')
  })
  queryBody.schoolAttr = changeSchoolAttr.toString()
  return queryBody
}
